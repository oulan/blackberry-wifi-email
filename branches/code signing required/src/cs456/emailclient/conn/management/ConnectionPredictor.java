			...
			try
			{
				...
				if(running)
				{
					...
					if(lastLocation != null)
					{
						if(!worker.isRunning())
						{
							// this is the case where we sum the recent GPS signals
							if(!WiFiStatus.WiFiIsOn())
							{
								if( lsQueue.sumNearestPoints(lastLocation) > PREDICT_TURN_ON_SIZE)
								{
							...
							else if(WiFiStatus.WiFiIsOn() )
							{
								//this is the case when we want to predict:
								//	- starting the PopWorker
								//	- OR
								//	- disabling the wifi
								if( lsQueue.sumNearestPoints(lastLocation) < PREDICT_TURN_OFF_SIZE)
								{
									WiFiStatus.WiFiTurnOff();
								}
								else
								{
									worker.StartPopRetriever();
